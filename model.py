...
class Attention(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # (B, T, D) * (D, A) -> (B, T, A)
        v = torch.tanh(
            torch.tensordot(inputs, self.w_omega, dims=([2], [0])) + self.b_omega
        )
        ...
        output = torch.sum(
            inputs * alphas.unsqueeze(-1), dim=1
...
class GCRAM_PyTorch(nn.Module):
    def __init__(
        ...
        self.conv_block = nn.Sequential(
            ...
            nn.MaxPool2d(kernel_size=(1, pool_width), stride=(1, pool_stride)),
        )
    ...
    def forward(self, x, batch_size, num_timestep):
        ...
        x = x.view(
            batch_size, num_timestep, -1
        )  # Reshape: (batch_size, num_timestep, features)

        # Bi-LSTM
        rnn_output, _ = self.lstm(
            x
        )  # -> (batch_size, num_timestep, n_hidden_state * 2)

        # Attention
        attn_output, _ = self.attention(
            rnn_output
        )  # -> (batch_size, n_hidden_state * 2)
